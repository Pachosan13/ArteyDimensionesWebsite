  ...
  return (
    <section className="py-16 md:py-24 bg-[#F5F5F5]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-[clamp(1.5rem,2.8vw,2.5rem)] font-bold text-[#2D2D2D] mb-8">{g.heading}</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {images.map((src, i) => (
            <button
              ...
              aria-label={`${g.zoomAria} ${i + 1} ${g.ofLabel} ${images.length}`}
            ...
            >
              <img
                src={src}
                alt={getImageAlt(src, i, g, projectTitle)}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={`${projectTitle || g.heading} — ${g.enlargedAria}`}
          ...
          >
            ...
            <div className="relative flex-1 flex items-center justify-center px-4 sm:px-16 overflow-hidden">
              ...
              <AnimatePresence initial={false} custom={direction} mode="popLayout">
                <motion.img
                  key={current}
                  src={images[current]}
                  alt={getImageAlt(images[current], current, g, projectTitle)}
                  custom={direction}
            ...
            {projectTitle && <p className="text-center text-white/70 text-sm pb-3 px-4">{projectTitle}</p>}
              ...
              >
                {images.map((src, i) => (
                  <button
                    ...
                    aria-label={`${g.goToAria} ${i + 1}`}
                    aria-current={i === current}
                    className={`shrink-0 w-20 h-14 sm:w-24 sm:h-16 rounded-lg overflow-hidden transition-all duration-200 ${
                      i === current ? "ring-2 ring-brand opacity-100 scale-100" : "opacity-50 hover:opacity-90 scale-95"
                    }`}
                  >
