...
import { localizeProject, projects } from '../i18n/data';

const ProjectsGrid: React.FC = () => {
  const { locale, t, path } = useI18n();
  const items = projects.map((p) => localizeProject(p, locale));

  return (
    <section id="portafolio" className="py-20 bg-[#F5F5F5]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <p className="text-sm font-semibold tracking-[0.3em] text-brand uppercase">{t.projectsSection.eyebrow}</p>
          <h2 className="text-[clamp(2rem,4vw,3.5rem)] font-bold text-[#1F1F1F] mb-4">
            {t.projectsSection.headingPre}{' '}
            <span className="border-b-4 border-brand">{t.projectsSection.headingHighlight}</span>
          </h2>
          <p className="text-lg text-[#4B4B4B]/80 max-w-3xl mx-auto">{t.projectsSection.subheading}</p>
        </div>
          ...
          {items.map((project, index) => (
            ...
            >
              <Link
                ...
                className="block"
                aria-label={`${t.projectsSection.hoverCta}: ${project.title}`}
              >
                <div className="relative h-72 md:h-80 overflow-hidden">
                  ...
                  >
                    <span className="text-white font-semibold text-lg inline-flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition delay-100">
                      <span>{t.projectsSection.hoverCta}</span>
                ...
                <div className="p-6 text-center">
                  <div className="text-brand text-sm font-semibold uppercase mb-1 tracking-wide">{project.category}</div>
                  <h3 className="text-2xl font-bold text-[#2D2D2D]">{project.title}</h3>
                </div>
              </Link>
            </motion.div>
          ))}
