...
const FeaturedProjectsSection: React.FC<FeaturedProjectsSectionProps> = ({ filterSlugs }) => {
  ...
  const items = (filterSlugs ? projects.filter((p) => filterSlugs.includes(p.slug)) : projects).map((p) =>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          <h2 className="text-4xl md:text-5xl font-bold text-[#4B4B4B] mb-4">
            {t.featuredProjects.headingPre}{' '}
            <span className="border-b-4 border-brand">{t.featuredProjects.headingHighlight}</span>
          </h2>
          <p className="text-lg text-[#4B4B4B]/70">{t.featuredProjects.subheading}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {items.map((project, index) => (
            ...
            >
              <Link to={path('projects', project.slug)} className="block">
                <div className="relative h-72 md:h-80 overflow-hidden">
                  ...
                  >
                    <span className="text-white font-semibold text-lg inline-flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition delay-100">
                      <span>{t.featuredProjects.hoverCta}</span>
